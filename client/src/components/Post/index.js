...
const Post = ({ user }) => {
  ...
  const getPost = () => {
    ...
    axios
      ...
      .then((res) => {
        if (
          res.data?.post?.isBlacklisted &&
          !(res.data?.post?.createdBy === user?._id || user?.role === "admin")
        ) {
          alert("Post is blacklisted!");
          navigate("/home");
          return;
        }
  ...
  useEffect(() => {
    if (post) {
      ...
      let find = post.votes.find((vote) => vote === user?._id);
  ...
  const handleBlacklist = () => {
    axios
      .patch(`/api/posts/blacklist/${post._id}`)
      .then((res) => {
        console.log(res);
        window.alert("Blacklisted successfully");
        const newPost = { ...post, isBlacklisted: true };
        setPost(newPost);
      })
      .catch((err) => {
        alert(err.response.data);
  ...
  const handleWhitelist = () => {
    axios
      .patch(`/api/posts/whitelist/${post._id}`)
      .then((res) => {
        console.log(res);
        window.alert("Whitelisted successfully");
        const newPost = { ...post, isBlacklisted: false };
        setPost(newPost);
      })
      .catch((err) => {
        alert(err.response.data);
  ...
  const handleCommentDelete = (id) => {
    axios
      ...
      .then((res) => {
        ...
        const index = post.comments.findIndex((c) => c._id === id);
            ...
            <Grid container>
              ...
              >
                <Stack direction="row" spacing={2}>
                  {user && post && user.role === "admin" && (
                    <Tooltip
                      title={
                        post.isBlacklisted ? "Whitelist Post" : "Blacklist Post"
                      }
                    >
                      <IconButton
                        onClick={
                          post.isBlacklisted ? handleWhitelist : handleBlacklist
                        }
                      >
                        {post.isBlacklisted ? <CheckIcon /> : <BlockIcon />}
                      </IconButton>
                    </Tooltip>
                  )}
